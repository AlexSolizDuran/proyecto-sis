...
<div class="container">
    ...
    <p><strong>Precio de compra:</strong> {{ $compra->precio_compra }}</p>
    <p><strong>Precio logística:</strong> {{ $compra->precio_logistica }}</p>
        ...
        <tbody>
            ...
            @else
                @foreach ($registros as $registro)
                    <tr>
                        <td>{{ $registro->getAttribute('cod_calzado') ?? 'No disponible' }}</td>
                        <td>{{ $registro->getAttribute('cantidad') ?? 'No disponible' }}</td>
                        <td>{{ $registro->getAttribute('precio_compra') ?? 'No disponible' }}</td>
                    </tr>
                @endforeach
            @endif
            <tr>
